...
var emp_det = require('./emp_details'); 
var edu_details = require('./edu_details');
var language=require('./language');
var prefrences=require('./prefrences'); 
var refrences=require('./refrences');
...
const conn = mysql.createConnection({
  ...
  user: "root",
  password: "root",
  database: "12march_job_app_insert"
});
...
app.post('/update/:id',async(req,res)=>{
 ...
 // Technologies...
//  let te=await query(`select technology_id as tech_id from technologies where emp_id in (${req.params.id})` );
//  var tech_ar=[];
//  tech_ar.push(formData.tech1);
//  tech_ar.push(formData.tech2);
//  tech_ar.push(formData.tech3);
//  tech_ar.push(formData.tech4);
//  console.log('teee',te);
//  console.log(tech_ar);
//  let technology=formData.technology;
//  console.log(technology);
//  for(let i=0;i<technology.length;i++){
//    if(te[i]){
//      let q5=`update technologies
//      set emp_id='${req.params.id}',technology_name='${formData.technology[i]}',language_level='${tech_ar[i]}'
//      where emp_id='${req.params.id}' and technology_id='${te[i].tech_id}';`
//      await query(q5);
//    }
//    else{
//      let q6=`insert into technologies(emp_id,technology_name,language_level)
//      values('${req.params.id}','${formData.technology[i]}','${tech_ar[i]}')`
//      await query(q6);
//    }
//  }
 ...
 // Refrence...
//  let re=await query(`select refrence_id as ref_id from refrences where emp_id in (${req.params.id})`);
//  console.log('re',re);
//  let ref_name=formData.ref_name;
//  console.log('ref_name.length'.ref_name.length);
//  for(let i=0;i<ref_name.length;i++){
//    if(re[i]){
//      q7=`UPDATE  refrences
//      set emp_id='${req.params.id}',ref_name='${formData.ref_name[i]}',contact_no='${formData.contact_number[i]}',relation='${formData.relation[i]}'
//      where emp_id='${req.params.id}' and refrence_id='${re[i].ref_id}';`
//      await  query(q7);
//    }
//    else{
//        let q8=`insert into refrences(emp_id,ref_name,contact_no,relation)
//        values('${req.params.id}','${formData.ref_name[i]}','${formData.contact_number[i]}','${formData.relation[i]}')`
